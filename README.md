...
- **Delivery rendering with custom components** — the catch-all route (and the home page, if a page is published at `/`) fetches a page by slug and renders its blocks server-side. [`src/contoprix/components.ts`](./src/contoprix/components.ts) registers hand-built components for every component type in the schema (`hero_banner`, `capabilities_section`, `button`, `seo`, plus `blog_post-detail`/`blog_post-list`); any type added to the schema after the last `contoprix pull` falls back to the SDK's schema-driven `GenericBlockRenderer` until you run `contoprix components` again. See [How rendering works](#how-rendering-works) for the exact resolution order.
- **A CMS-driven home page** (`/`) — attempts to fetch a published page at `/` and render it through the normal component/generic-renderer resolution; if none is published yet, falls back to a static intro plus a live "Latest from the blog" section (`blog_post` entries fetched via `client.content.list()`, rendered through the registered `BlogPostList` component). Publish a page with a `hero_banner` or `capabilities_section` block in the Contoprix admin and it renders automatically — no code change needed.
...
│   ├── page.tsx                      # home page: renders a published "/" page if one exists, else a static intro + blog list
...
├── src/contoprix/
│   ├── components.ts                 # ComponentRegistry — hero_banner, capabilities_section, button, seo, blog_post-*
...
├── src/components/contoprix/         # custom overrides registered in src/contoprix/components.ts
│   ├── HeroBanner.tsx, CapabilitiesSection.tsx, Button.tsx, Seo.tsx
│   ├── BlogPostDetail.tsx, BlogPostList.tsx, ContentList.tsx
│   ├── richtext.tsx                  # safe ProseMirror-JSON -> React renderer (no dangerouslySetInnerHTML)
│   └── icons.tsx                     # resolves an icon-key text field to a lucide-react icon
└── src/lib/contoprix/
    ├── errors.ts                     # isNotFoundError() -- getContoprixPage() rejects, never resolves null, on a 404
